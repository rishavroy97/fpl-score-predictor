...
df = pd.read_csv('../gk_result_files/sorted_names.csv')

columns = ['mins-pm', 'team', 'opponent', 'cs-pm','conceded-pm', 'saves-pm','penalty-saves-pm', 'yellow-cards-pm', 'red-cards-pm', 'bonus-pm', 'current-form', 'total']
...
for i in range(start_index, len(df)):
    ...
    if number_of_matches == 0 or i == 0 or starting_row['playerId'] != current_player:
        ...
        new_row['mins-pm'] = 0
        new_row['conceded-pm'] = 0
        new_row['saves-pm'] = 0
        new_row['penalty-saves-pm'] = 0
        new_row['cs-pm'] = 0
        ...
        for j in range(current_player_starting_index, i):
            ...
            total_conceded += df.iloc[j]['goals-conceded']
            total_saves += df.iloc[j]['saves']
            total_penalty_saves += df.iloc[j]['penalty-saves']
        ...
        new_row['conceded-pm'] = total_conceded / number_of_matches
        new_row['cs-pm'] = total_cs / number_of_matches
        new_row['saves-pm'] = total_saves / number_of_matches
        new_row['penalty-saves-pm'] = total_penalty_saves / number_of_matches
...
print new_df
new_df.to_csv('../gk_result_files/gk_data.csv', index=False)